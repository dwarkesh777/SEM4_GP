...
import { MessageCircle } from "lucide-react"; // Fallback icon in case SVG isn't used, but we'll use an SVG

const WhatsAppChat = () => {
    const phoneNumber = "7859988312";
    
    // Check if the user is on mobile to adjust the link for app opening
    const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
    const whatsappLink = isMobile 
        ? `whatsapp://send?phone=91${phoneNumber}` 
        : `https://web.whatsapp.com/send?phone=91${phoneNumber}`;

    const fallbackLink = `https://wa.me/91${phoneNumber}`;
