...
import { Building2, Mail, Lock, User, ArrowRight, Camera, RotateCcw, Check, Loader2 } from "lucide-react";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4">
            ...
            >
                <div className="bg-indigo-600 px-6 py-5 text-center">
                    ...
                    <p className="text-indigo-200 text-sm mt-1">Take a clear photo of your face</p>
                </div>
                ...
                <div className="p-6">
                    ...
                    <div className="flex flex-col gap-3">
                        {!captured ? (
                            <Button onClick={capture} className="w-full py-6 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-lg">
...
// ─── Owner Signup Page ────────────────────────────────────────────────────────
const OwnerSignup = () => {
    const [formData, setFormData] = useState({ fullName: "", businessName: "", email: "", phone: "", password: "", confirmPassword: "" });
    ...
    const handleCapture = async (facePhotoUrl) => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`${API_URL}/api/auth/signup/owner`, {
                method: "POST",
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md">
                <Card className="border-none shadow-2xl shadow-indigo-500/5 rounded-[2rem] overflow-hidden">
                    <CardHeader className="space-y-1 pb-8 text-center bg-white">
                        ...
                        <CardDescription className="text-slate-500 font-medium tracking-tight">List your properties and grow your business.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6 pt-2 bg-white px-8">
                        <form onSubmit={handleSubmit} className="space-y-4">
                            {[
                                { id: "fullName", label: "Full Name", icon: User, placeholder: "John Doe" },
                                { id: "businessName", label: "Business / Property Name", icon: Building2, placeholder: "Doe Residences" },
                                { id: "email", label: "Business Email", icon: Mail, type: "email", placeholder: "partner@company.com" },
                                { id: "phone", label: "Phone Number", icon: User, placeholder: "9876543210" },
                                ...
                                <div key={id} className="space-y-1">
                                    ...
                                    <div className="relative group">
                                        ...
                                        <Input
                                            ...
                                            className={`pl-11 py-6 rounded-2xl border-slate-200 focus:ring-indigo-500/20 ${errors[id] ? "border-red-400" : ""}`}
                                            value={formData[id]}
                            ...
                            <Button type="submit" disabled={loading}
                                className="w-full py-7 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-lg shadow-xl shadow-indigo-600/20 transition-all active:scale-[0.98] mt-2">
                                {loading ? <><Loader2 className="mr-2 w-5 h-5 animate-spin" /> Processing...</> : <>Register Business <ArrowRight className="ml-2 w-5 h-5" /></>}
                            </Button>
                        </form>
                    </CardContent>
